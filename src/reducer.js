...
const ScheduleReducer = (state = DEFAULT_STATE, action) => {
    ...
    switch (type) {
        ...
        case RECEIVE_SCHED_SUMMIT: {
            ...
            const schedule_start = summit.schedule_start_date ? epochToMomentTimeZone(summit.schedule_start_date, summit.time_zone_id).format('YYYY-MM-D') : null;
            const summit_start = epochToMomentTimeZone(summit.start_date, summit.time_zone_id).format('YYYY-MM-D');
